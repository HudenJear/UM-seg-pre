...
class SequentialModel(BaseModel):
    ...
    def __init__(self, opt):
        super(SequentialModel, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        # open grad
          ...
          # real
          self.pred = self.net_d(self.output.detach())
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        if self.opt['val']['save_prediction']:
            if self.opt['is_train']:
                ...
                sav_path = osp.join(self.opt['path']['visualization'], f'0_prediction_res.csv')
            else:
                if self.opt['val']['suffix']:
                    sav_path = osp.join(self.opt['path']['visualization'], dataset_name,f'0_prediction_{self.opt["val"]["suffix"]}.csv')
                else:
                    sav_path = osp.join(self.opt['path']['visualization'], dataset_name,f'0_prediction_res.csv')
            sav_csv = {
                "file_name": name_list,
