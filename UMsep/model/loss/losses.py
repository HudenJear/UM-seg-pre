...
class DiceLoss(nn.Module):
    def __init__(self, smooth=1., loss_weight=1.0):
        super(DiceLoss, self).__init__()
        self.smooth = smooth
        self.loss_weight=loss_weight

    def forward(self, output, target):
        # output=(output*255).int()
        # target=target.long()
        # # print(target[0,0,180:210,180:210])
...
class FocalLoss(nn.Module):
    def __init__(self, gamma=2, alpha=None,loss_weight=1.0, size_average=True):
...
class GradLoss(nn.Module):
    ...
    def conv_gauss(self, img):
        ...
        # print(img.shape)
        img = F.pad(img.float(), (kw//2, kh//2, kw//2, kh//2), mode='replicate')
        # print(img.shape)
...
#        diff = current - filtered
    ...
    def forward(self, x, y):
        # y = make_one_hot(y.unsqueeze(dim=1), classes=x.size()[1])
        # x = F.softmax(x, dim=1)
